        ...
        <CardContent>
          <div className="flex items-start justify-between">
            <div className={`rounded-lg p-2 ${accent ?? "bg-primary/10"}`}>
              <Icon className={`size-5 ${accent ? "text-white" : "text-primary"}`} />
            </div>
    ...
    <Link href={href} className="block w-60 shrink-0">
      <Card className="overflow-hidden transition-shadow hover:shadow-md h-full">
        ...
        <CardContent className="p-3">
          <p className="truncate text-sm font-medium leading-tight">{listing.title}</p>
          ...
          <div className="mt-2 flex items-center justify-between">
            ...
            {!isShortlet && (listing.bedrooms != null || listing.bathrooms != null) && (
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                {listing.bedrooms != null && (
                  <span className="flex items-center gap-0.5">
                    <IconBed className="size-3" />{listing.bedrooms}
                  </span>
                )}
                {listing.bathrooms != null && (
                  <span className="flex items-center gap-0.5">
                    <IconBath className="size-3" />{listing.bathrooms}
                  </span>
                )}
              </div>
            )}
  ...
  return (
    <div className="flex items-center justify-between">
      ...
      <Button variant="ghost" size="sm" asChild>
        <Link href={href}>{label} <IconArrowRight className="ml-1 size-3" /></Link>
      </Button>
    </div>
  )
}
...
const QUICK_ACTIONS = [
  ...
  { label: "Rental Payments", icon: IconCurrencyNaira, href: "/rental-payments" },
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={memberSince ? `Member since ${memberSince}` : "Your housing dashboard"}
      />

      {/* Stat cards */}
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard icon={IconBookmark} label="Saved Listings" value={stats?.saved ?? 0} href="/saved" loading={loading} />
        <StatCard icon={IconClipboardList} label="Applications" value={stats?.applications ?? 0} href="/applications" loading={loading} />
        <StatCard icon={IconCalendar} label="Bookings" value={stats?.bookings ?? 0} href="/bookings" loading={loading} />
        <StatCard
          icon={IconBell}
        ...
        <Link href="/wallet">
          <Card className="h-full cursor-pointer transition-shadow hover:shadow-md">
            <CardContent>
              ...
              <div className="mt-4">
                {loading ? <Skeleton className="h-7 w-28" /> : <p className="text-2xl font-bold">{fmt(stats?.walletBalance ?? 0)}</p>}
                <p className="mt-1 text-sm text-muted-foreground">Sage Nest Balance</p>
              </div>
            </CardContent>
          </Card>
        </Link>
        ...
        <Link href="/firstkey">
          <Card className="h-full cursor-pointer transition-shadow hover:shadow-md">
            <CardContent>
              ...
              <div className="mt-4">
                {loading ? <Skeleton className="h-7 w-28" /> : <p className="text-2xl font-bold">{fmt(stats?.savings.totalBalance ?? 0)}</p>}
                <p className="mt-1 text-sm text-muted-foreground">FirstKey Savings</p>
              </div>
            </CardContent>
          </Card>
        </Link>
        ...
        <Link href="/firstkey">
          <Card className="h-full cursor-pointer transition-shadow hover:shadow-md">
            <CardContent>
              ...
              <div className="mt-4">
                {loading ? <Skeleton className="h-7 w-28" /> : <p className="text-2xl font-bold text-emerald-600">+{fmt(stats?.savings.totalInterest ?? 0)}</p>}
                <p className="mt-1 text-sm text-muted-foreground">Interest Earned · 12% p.a.</p>
              </div>
            </CardContent>
          </Card>
        </Link>
      ...
      <div className="space-y-3">
        <SectionHeader title="Explore Listings" href="/" />
        {loading ? (
      ...
      {/* Featured Shortlets */}
      <div className="space-y-3">
        <SectionHeader title="Shortlets Near You" href="/?type=SHORTLET" label="See all shortlets" />
        {loading ? (
          <HorizontalCardSkeleton />
                  ...
                  <Link key={plan.id} href={`/firstkey/${plan.id}`}>
                    <div className="flex items-center justify-between rounded-lg border p-3 transition-colors hover:bg-muted/40">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-medium">{plan.planName ?? "FirstKey Plan"}</p>
                          <p className="text-xs text-muted-foreground">{fmt(plan.totalDeposited + plan.interestEarned)} saved</p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-xs font-medium text-emerald-600">+{fmt(plan.interestEarned)} interest</p>
                        <Badge
                          variant="outline"
                          className={`mt-0.5 text-[10px] ${
                            plan.status === "ACTIVE" ? "border-emerald-200 text-emerald-700"
                            : plan.status === "MATURED" ? "border-blue-200 text-blue-700"
          ...
          <CardContent className="space-y-3">
            {loading ? (
              ...
              <div className="py-8 text-center text-sm text-muted-foreground">
                No applications yet.{" "}
                <Link href="/" className="text-primary hover:underline">Browse listings</Link>
              </div>
            ) : (
                  ...
                  <div key={app.id} className="flex items-center gap-3 rounded-lg border p-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="flex items-center gap-1 text-xs text-muted-foreground">
                        <IconMapPin className="size-3" />{app.listing.area}, {app.listing.state}
                      </p>
                    </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {loading ? (
              ...
              <div className="py-8 text-center text-sm text-muted-foreground">
                No bookings yet.{" "}
                <Link href="/?type=SHORTLET" className="text-primary hover:underline">Find a shortlet</Link>
              </div>
            ) : (
          ...
          <CardContent className="space-y-3">
            {loading ? (
              ...
              stats.upcomingTours.map((tour) => {
                const badge = TOUR_STATUS[tour.status] ?? { label: tour.status, className: "" }
                  ...
                  <div key={tour.id} className="flex items-center gap-3 rounded-lg border p-3">
                    <div className="relative size-12 shrink-0 overflow-hidden rounded-lg bg-muted">
                      {tour.listing.photos[0] && (
                        <Image src={tour.listing.photos[0]} alt={tour.listing.title} fill className="object-cover" />
                      )}
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-medium">{tour.listing.title}</p>
                      <p className="flex items-center gap-1 text-xs text-muted-foreground">
                        <IconMapPin className="size-3" />{tour.listing.area}, {tour.listing.state}
                      </p>
                      {date && (
